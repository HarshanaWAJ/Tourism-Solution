...
export default function Hero() {
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-teal-950 via-teal-900 to-teal-950 text-sand-50">
      {/* Decorative animated backdrop — no external images/video, so nothing to break */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
        ...
        />
        <svg
          className="absolute bottom-0 left-0 w-full text-teal-900/60"
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 pt-20 pb-28 flex flex-col items-center text-center gap-8">
        <motion.div
          initial={{ opacity: 0, y: -12 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
        >
          <span className="inline-flex items-center gap-2 rounded-full border border-saffron-400/40 bg-white/5 px-4 py-1.5 text-sm font-semibold tracking-wide text-saffron-300">
            <Sparkles className="w-4 h-4" />
            Sri Lanka, without the guesswork
          </span>
        </motion.div>
            ...
            {words.map((word, index) => (
              <motion.span
                ...
                animate={
                  wordIndex === index
                    ? { opacity: 1, y: 0 }
                    : { opacity: 0, y: wordIndex > index ? -40 : 40 }
                }
        ...
        <div className="flex flex-wrap justify-center gap-3">
          <Link
            to="/discover"
            className="inline-flex items-center gap-2 bg-saffron-500 text-teal-950 rounded-full px-6 py-3 font-medium hover:bg-saffron-400 transition"
          >
            Start exploring <ArrowRight className="w-4 h-4" />
          </Link>
          <Link
            to="/register"
            className="inline-flex items-center gap-2 border border-white/20 rounded-full px-6 py-3 font-medium text-sand-50 hover:bg-white/10 transition"
          >
            List your business
          </Link>
        </div>
